...
Improvements:
- Added customization for the `SAM` parameter `--build-dir`:

  _Run/Debug Configuration_ -> Your Lambda -> _SAM CLI_ -> _Build Dir_

  If the parameter is not set, autogenerated directory will be used.

Execute the following command to build an artifact.

```bash
./gradlew -PideProfileName=2019.3 -PtoolkitVersion=1.13.1 -PskipRider buildPlugin
```

Then look at the directory `build\distributions`.
